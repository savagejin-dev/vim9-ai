...
![vim9-ai demo](./demo.gif)

To get an idea what is possible to do with AI commands see the [prompts](https://github.com/madox2/vim9-ai/wiki/AI-prompts#prompts) on the [Community Wiki](https://github.com/madox2/vim9-ai/wiki)
...
This plugin uses OpenAI's API to generate responses. All network requests are
performed by the external `curl` executable - the plugin itself is pure Vim9
script and does not require Python.
...
`vim9-ai` can now be extended with custom provider plugins.
...
- Vim 9.0.1669 or newer (requires Vim9 script support, including `import autoload`)
- [curl](https://curl.se/) executable available in `PATH`
- Optionally PowerShell (Windows) or `base64` (Unix) for the image features
...
The default api key file location is `~/.config/openai.token`, but you can change it by setting the `g:vim9_ai_token_file_path` in your `.vimrc` file:

```vim
let g:vim9_ai_token_file_path = '~/.config/openai.token'
```
...
git clone https://github.com/madox2/vim9-ai.git ~/.vim/pack/plugins/start/vim9-ai

# neovim
mkdir -p ~/.local/share/nvim/site/pack/plugins/start
git clone https://github.com/madox2/vim9-ai.git ~/.local/share/nvim/site/pack/plugins/start/vim9-ai
```
...
If you are interested in more tips or would like to level up your Vim with more commands like [`:GitCommitMessage`](https://github.com/madox2/vim9-ai/wiki/Custom-commands#suggest-a-git-commit-message) - suggesting a git commit message, visit the [Community Wiki](https://github.com/madox2/vim9-ai/wiki).

## Providers

This is the list of 3rd party provider plugins allowing to use different AI providers.

The bundled `openai` provider is implemented in pure Vim9 script
(`autoload/vim9_ai_provider_openai.vim`). Third-party provider plugins written
for the previous Python based versions are **not** compatible anymore.

A Vim9 provider is an autoload script registered through
`vim9_ai_provider#Register(name, {'autoload': ..., 'default_options': {...}})`
that exports `Request(provider_name, command_type, raw_options, messages,
callbacks)` and `RequestImage(provider_name, command_type, raw_options,
prompt)` functions.
...
```vim
let g:vim9_ai_roles_config_file = '/path/to/my/roles.ini'
```
...
# NOTE: this role takes precedence over g:vim9_ai_chat, g:vim9_ai_complete, g:vim9_ai_edit
```
...
let g:vim9_ai_roles_config_file = s:plugin_root . "/roles-example.ini"

" custom token file location
let g:vim9_ai_token_file_path = "~/.config/openai.token"

" custom fn to load token, e.g. "g:GetAIToken()"
let g:vim9_ai_token_load_fn = ""

" set proxy for all AI requests
let g:vim9_ai_proxy = 'http://your-proxy-server:port'

" enable/disable asynchronous AIChat (enabled by default)
let g:vim9_ai_async_chat = 1
...
" debug settings
let g:vim9_ai_debug = 0
let g:vim9_ai_debug_log_file = "/tmp/vim9_ai_debug.log"
...
See some cool projects listed in [Custom APIs](https://github.com/madox2/vim9-ai/wiki/Custom-APIs) section on the [Community Wiki](https://github.com/madox2/vim9-ai/wiki).
...
You might find useful a [collection](https://github.com/madox2/vim9-ai/wiki/Custom-commands) of custom commands on the [Community Wiki](https://github.com/madox2/vim9-ai/wiki).

To create a custom command, you can call `AIRun`, `AIEditRun` and `AIChatRun`
functions. The signature of each function is:

```
vim9_ai#AIRun(uses_range, firstline, lastline, config, ...prompt)
```

where `firstline`/`lastline` are the (optional) line range. For example:
...
function! GitCommitMessageFn()
  ...
  \  },
  \}
  call vim9_ai#AIRun(l:range, l:firstline, l:lastline, l:config, l:prompt)
endfunction
command! GitCommitMessage call GitCommitMessageFn()


" custom command that provides a code review for selected code block
function! CodeReviewFn(range, firstline, lastline)
  ...
  \  },
  \}
  call vim9_ai#AIChatRun(a:range, a:firstline, a:lastline, l:config, l:prompt)
endfunction
command! -range -nargs=? AICodeReview <line1>,<line2>call CodeReviewFn(<range>, <line1>, <line2>)
```

## Contributing

Contributions are welcome! Please feel free to open a pull request, report an issue, or contribute to the [Community Wiki](https://github.com/madox2/vim9-ai/wiki).
...
[MIT License](https://github.com/madox2/vim9-ai/blob/main/LICENSE)
